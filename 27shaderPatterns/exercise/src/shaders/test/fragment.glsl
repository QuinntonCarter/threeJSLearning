...
void main()
{
    ...
    // pattern 3.5
    // float strength = vUv.x; // used in 3.5 - 4
    // gl_FragColor = vec4(vUv.x, vUv.x, vUv.x, 1.0);
    ...
    // float strength = 1.0 - vUv.y; // used in 5
    // gl_FragColor = vec4(strength, strength, strength, 1.0);
    ...
    // float strength = mod(vUv.y * 10.0, 1.0);
    // float strength = strength < 0.5 ? strength = 0.0 : strength= 1.0; // bad for perfomance
    // strength = step(0.5, strength); // better for performance
    // gl_FragColor = vec4(strength, strength, strength, 1.0);
